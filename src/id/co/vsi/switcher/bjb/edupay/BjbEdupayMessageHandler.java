...
import static id.co.vsi.switcher.bjb.edupay.ReceiptFormat.buildReceiptFormat;
...
public abstract class BjbEdupayMessageHandler extends BJBJsonHandler {
    public static String cProcCodeInquiry = "321000";
    public static String cProcCodePayment = "521000";

    public BjbEdupayMessageHandler(JSONPlugin pParentPlugin) {
        super(pParentPlugin);
    }

    protected JSONObject performEDUPAYMessage(final JSONObject pRequestMessage, final String pRF, final String pProcessingCode) {
        ...
        if (isSimulateISO) {
            tResponseInquiryPayment = new SimulateResponse().getIsoResponse(tRequestInquiryPayment, tMP);
        } else {
        ...
        if (tResponseInquiryPayment.getDataElements().containsKey(39) && tResponseInquiryPayment.getValueForDataElement(39).equals("00")) {
            ...
            if (pRequestMessage.getString(cMTIKey).equals(c2100) && tResponseInquiryPayment.getDataElements().containsKey(61)) {
                ...
                tJsonPrivateData.put("NOMOR_TAGIHAN", tJsonPrivateData.getString("NOMOR_TAGIHAN").trim());
                tJsonPrivateData.put("NAMA_SISWA", tJsonPrivateData.getString("NAMA_SISWA").trim());
                tJsonPrivateData.put("NAMA_FAKULTAS", tJsonPrivateData.getString("NAMA_FAKULTAS").trim());
                tJsonPrivateData.put("NAMA_JURUSAN", tJsonPrivateData.getString("NAMA_JURUSAN").trim());
                tJsonPrivateData.put("TAHUN_AKADEMIK", tJsonPrivateData.getString("TAHUN_AKADEMIK").trim());
                tJsonPrivateData.put("PIN_DATA", tJsonPrivateData.getString("PIN_DATA").trim());
                tJsonPrivateData.put("INFO_DATA", tJsonPrivateData.getString("INFO_DATA").trim());
                
                String tJenisPembayaran = tJsonPrivateData.getString("JENIS_PEMBAYARAN").trim();
                if (tJenisPembayaran.equalsIgnoreCase("0")) {
                    tJenisPembayaran = "Tagihan dibayar secara full";
                } else if (tJenisPembayaran.equalsIgnoreCase("1")){
                    tJenisPembayaran = "Tagihan dapat dicicil";
                } else {
                    tJenisPembayaran = tJsonPrivateData.getString("JENIS_PEMBAYARAN").trim();
                }
                tJsonPrivateData.put("JENIS_PEMBAYARAN", tJenisPembayaran);
                
                String tJenisTagihan = tJsonPrivateData.getString("JENIS_TAGIHAN").trim();
                if (tJenisTagihan.equalsIgnoreCase("REG")) {
                    tJenisTagihan = "Pembayaran Reguler";
                } else if (tJenisTagihan.equalsIgnoreCase("PMB")){
                    tJenisTagihan = "Pembayaran Mahasiswa Baru (Pembelian PIN)";
                } else {
                    tJenisTagihan = tJsonPrivateData.getString("JENIS_TAGIHAN").trim();
                }
                tJsonPrivateData.put("JENIS_TAGIHAN", tJenisTagihan);
                
                BigInteger admin = new BigInteger(tResponseInquiryPayment.getValueForDataElement(57).trim());
                BigInteger amount = new BigInteger(tJsonPrivateData.getString("JUMLAH_TAGIHAN"));
                ...
                tJsonPrivateData.put("BILLER_NAME", getBillerName(tJsonPrivateData.getString("KODE_UNIVERSITAS").trim(), tResponseMessage.getString(cModuleCodeKey)));
            ...
            if (tResponseMT.equals(c2210)) {
                ...
                tJsonPrivateData.put("REF_NUM", tResponseInquiryPayment.getDataElements().containsKey(37)
    ...
    private ISO8583Message buildISO8583Message(final String pProcessingCode, final JSONObject pRequestMessage, Map<String, String> pAccountData) {
        ...
        tPaymentRequestMessage.setValueForDataElement(22, "021");
        ...
        tPaymentRequestMessage.setValueForDataElement(60, "H2H");
        if (pProcessingCode.equals(cProcCodeInquiry)) {
            String tBiller = rightSpacePadding(tMP.getString("BILLER"), 5);
            String tBillId = rightSpacePadding(tMP.getString("BILLID"), 20);
            String tRoutingToBit61 = tBiller + tBillId;
            tPaymentRequestMessage.setValueForDataElement(61, tRoutingToBit61);
        }
    ...
    public static JSONObject getParsePrivateDataBit61(final String tPrivateData, final JSONObject pModulParameter) {
        ...
        for (BjbEdupayPrivateDataBit61 tBjbPdamPrivateDataBit61 : BjbEdupayPrivateDataBit61.values()) {
    ...
    public static String getBit61DataValue(final BjbEdupayPrivateDataBit61 pBit61n, final String pBit61) {
        final int length = pBit61n.getLength();
        String tReturn = "";
        int start = 0;

        for (BjbEdupayPrivateDataBit61 tPdamRule : BjbEdupayPrivateDataBit61.values()) {
